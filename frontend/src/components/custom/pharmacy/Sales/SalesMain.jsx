...
export default function SalesMain({ clearTrigger, shouldOpenMedicineSuggDialog, setShouldOpenMedicineSuggDialog }) {
  ...
  const [customerInfo, setCustomerInfo] = useState({ name: "", phone: "", patientId: "" });
  ...
  const handleCreateSalesOrder = (e) => {
    ...
    dispatch(createSalesBill({ items : itemsArray, patientInfo : { ...customerInfo, patientId: customerInfo.patientId || null }, totals, paymentMethod, buyerName }));
  };
  ...
  const handlePatientSuggestionSelect = (suggestion) => {
    setCustomerInfo({
      ...
      patientId: suggestion.patientId || ""  // Include patientId
    });
    // Find the full patient object from patientlist
    const fullPatient = patientlist.find(p => p.patient._id === suggestion.patientId);
  ...
  const clearAllFields = () => {
    setCustomerInfo({ name: "", phone: "", patientId: "" });
    setItems([]);
  ...
  useEffect(() => {
    if (selectedPatient && shouldOpenMedicineSuggDialog) {
      setCustomerInfo({name: selectedPatient.patient.name,phone: selectedPatient.patient.contactNumber || "", patientId: selectedPatient.patient._id || ""});
